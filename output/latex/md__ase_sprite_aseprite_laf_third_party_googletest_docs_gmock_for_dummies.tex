\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md299}{}\doxysection{What Is g\+Mock?}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md299}
When you write a prototype or test, often it\textquotesingle{}s not feasible or wise to rely on real objects entirely. \mbox{\hyperlink{class_a}{A}} {\bfseries{mock object}} implements the same interface as a real object (so it can be used as one), but lets you specify at run time how it will be used and what it should do (which methods will be called? in which order? how many times? with what arguments? what will they return? etc).
...
\end{DoxyEnumerate}\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md300}{}\doxysection{Why g\+Mock?}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md300}
...
\end{DoxyItemize}\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md301}{}\doxysection{Getting Started}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md301}
g\+Mock is bundled with googletest.\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md302}{}\doxysection{A Case for Mock Turtles}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md302}
...
You don\textquotesingle{}t need to define these mock methods somewhere else -\/ the {\ttfamily MOCK\+\_\+\+METHOD} macro will generate the definitions for you. It\textquotesingle{}s that simple!\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md305}{}\doxysubsection{Where to Put It}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md305}
...
Admittedly, this test is contrived and doesn\textquotesingle{}t do much. You can easily achieve the same effect without using g\+Mock. However, as we shall reveal soon, g\+Mock allows you to do {\itshape so much more} with the mocks.\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md307}{}\doxysection{Setting Expectations}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md307}
The key to using a mock object successfully is to set the {\itshape right expectations} on it. If you set the expectations too strict, your test will fail as the result of unrelated changes. If you set them too loose, bugs can slip through. You want to do it just right such that your test can catch exactly the kind of bugs you intend it to catch. g\+Mock provides the necessary means for you to do it \char`\"{}just right.\char`\"{}\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md308}{}\doxysubsection{General Syntax}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md308}
...
\{\+: .callout .note\} {\bfseries{Note\+:}} Why do we use a macro to do this? Well it serves two purposes\+: first it makes expectations easily identifiable (either by {\ttfamily grep} or by a human reader), and second it allows g\+Mock to include the source file location of a failed expectation in messages, making debugging easier.\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md309}{}\doxysubsection{Matchers\+: What Arguments Do We Expect?}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md309}
...
{\ttfamily \+\_\+} is an instance of what we call {\bfseries{matchers}}. \mbox{\hyperlink{class_a}{A}} matcher is like a predicate and can test whether an argument is what we\textquotesingle{}d expect. You can use a matcher inside {\ttfamily EXPECT\+\_\+\+CALL()} wherever a function argument is expected. {\ttfamily \+\_\+} is a convenient way of saying \char`\"{}any value\char`\"{}.

In the above examples, {\ttfamily 100} and {\ttfamily 50} are also matchers; implicitly, they are the same as {\ttfamily Eq(100)} and {\ttfamily Eq(50)}, which specify that the argument must be equal (using {\ttfamily operator==}) to the matcher argument. There are many \mbox{\hyperlink{md__ase_sprite_aseprite_laf_third_party_googletest_docs_reference_matchers}{built-\/in matchers}} for common types (as well as \href{gmock_cook_book.md\#NewMatchers}{\texttt{ custom matchers}}); for example\+:
...
This works for all non-\/overloaded methods; if a method is overloaded, you need to help g\+Mock resolve which overload is expected by specifying the number of arguments and possibly also the \href{gmock_cook_book.md\#SelectOverload}{\texttt{ types of the arguments}}.\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md310}{}\doxysubsection{Cardinalities\+: How Many Times Will It Be Called?}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md310}
...
Instead of returning 100, 101, 102, ..., consecutively, this mock function will always return 100 as {\ttfamily n++} is only evaluated once. Similarly, {\ttfamily Return(new Foo)} will create a new {\ttfamily Foo} object when the {\ttfamily EXPECT\+\_\+\+CALL()} is executed, and will return the same pointer every time. If you want the side effect to happen every time, you need to define a custom action, which we\textquotesingle{}ll teach in the \mbox{\hyperlink{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_cook_book}{cook book}}.
...
(What if you care about the relative order of some of the calls, but not all of them? Can you specify an arbitrary partial order? The answer is ... yes! The details can be found \href{gmock_cook_book.md\#OrderedCalls}{\texttt{ here}}.)\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_StickyExpectations}{}\doxysubsection{All Expectations Are Sticky (\+Unless Said Otherwise)}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_StickyExpectations}
...
By the way, the other situation where an expectation may {\itshape not} be sticky is when it\textquotesingle{}s in a sequence -\/ as soon as another expectation that comes after it in the sequence has been used, it automatically retires (and will never be used to match any call).\hypertarget{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md312}{}\doxysubsection{Uninteresting Calls}\label{md__ase_sprite_aseprite_laf_third_party_googletest_docs_gmock_for_dummies_autotoc_md312}
\mbox{\hyperlink{class_a}{A}} mock object may have many methods, and not all of them are that interesting. For example, in some tests we may not care about how many times {\ttfamily Get\+X()} and {\ttfamily Get\+Y()} get called.
